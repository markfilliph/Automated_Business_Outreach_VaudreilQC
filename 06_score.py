...
def score_ownership_signal(row: pd.Series) -> float:
    ...
    Positive signals: Inc./Ltee in name (incorporated, likely owner-held),
    ...
    # "Les [X] Inc." or "[Name] Ltee" or "[Owner Name] et Fils"
    positive_patterns = [
        "ltee", "enr.", "et fils", "et filles", "& fils",
        "et associes", "et ass.", "& associes",
    ]
    ...
    if "canada" in name and any(s in name for s in ["inc", "ltd", "corp", "ltee"]):
        score -= 20.0
